...
let pixelWidth = 8;  // size of pixel area to capture (measured in square pixels) 
...
function draw() 
{
  ...
  for (let y = 0; y < video.height; y+=pixelWidth)  //for every row of pixels in canvas...
  {
    for (let x = 0; x < video.width; x+=pixelWidth)  //for every pixel in row...
    {
      ...
      if (mouseIsPressed)
      {
        ...
        circle(x + pixelWidth/2, y - pixelWidth/2, pixelWidth/2);
      }
      else
      {
        //draw square at pixel position 
        square(x + pixelWidth/2, y + pixelWidth/2, pixelWidth);
      }
    }
  }
}