...
function setup() 
{
  ...
  pixelDensity(1); //force pixel density to 1 for correct pixels[] array indexing
}
...
function draw() 
{
  ...
  if (pixelSize == 1) // if pixel size is equal to 1 pixel, which is the default,...
  {
    ...
    for (let y = 0; y < height; y++) //for each horizontal row of pixels in canvas...
    {
      for (let x = 0; x < width; x++) //for each pixel in row...
      {
        ...
        {
          bw = random(255);  // generate a random color value
        ...
        else 
        {
          ...
          pixels[index + 0] = y / 2;  //RED
          pixels[index + 1] = y / 4;  //GREEN
          pixels[index + 2] = (x * y) / mouseY;  //BLUE
          pixels[index + 3] = 255;    //ALPHA
        }
      }
    }
  ...
  else //if pixel size is greater than 1 pixel...
  {
    ...
    {
      for (let x = 0; x < width; x += pixelSize) //for every "pixel area" of pixelSize length...
      {
        //set pixels to color values based on their position
        r = y / 2;  //RED
        g = y / 4;  //GREEN
  ...
  stroke(255);
  strokeWeight(2);
  circle(mouseX, mouseY, mouseY * 0.8); //using mouseY * 0.8 as scaled diameter for simple z-axis distance effect
}
